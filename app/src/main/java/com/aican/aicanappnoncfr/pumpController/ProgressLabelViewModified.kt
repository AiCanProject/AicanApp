package com.aican.aicanappnoncfr.pumpController
...
import com.aican.aicanappnoncfr.utils.adjustAlpha
import com.aican.aicanappnoncfr.utils.toPx
import kotlin.math.log10

private const val EMPTY_CHAR = '#'

class ProgressLabelViewModified @JvmOverloads constructor(
    ...
    private val maxValueString = "3001"
    ...
    init {
        val typedArray = context.obtainStyledAttributes(attrs, R.styleable.ProgressLabelViewModified)
        try{
            setFontSize(typedArray.getDimension(R.styleable.ProgressLabelViewModified_textSize, 20F), false)
        }finally {
            typedArray.recycle()
        }
    }
...
//
//        var desiredTextSize = testTextSize * width / bounds.width()
//
//        paint.textSize = desiredTextSize
//
////        setMeasuredDimension(paint.measureText(maxValueString).toInt(), height)
//    }

    fun setFontSize(size: Float, updateDimensions: Boolean){
        paint.textSize = size
        paint.getTextBounds("999", 0, 3, viewBounds)
        if(updateDimensions) {
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        val w = (viewBounds.width()).toInt()
        val h = (viewBounds.height()+8F.toPx()).toInt()
